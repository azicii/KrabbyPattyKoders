    ...
    try {
      ...
      if (response.ok) {
        if (isLogin) {
          ...
          // Clear only the password/username for security, keep email for convenience?
          // Or clear all to let them start fresh:
          setFormData({ username: '', email: formData.email, password: '' });

          setTimeout(() => {
            setIsLogin(true); // Flip to the Login screen
            setSuccess('');   // Clear the green banner after they see it
          }, 2500);
        }
      } else {
