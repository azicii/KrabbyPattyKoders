...
export function HabitConfig({ 
  ...
  const [selectedFriend, setSelectedFriend] = useState<string | null>(preSelectedFriend?.id || null);
  ...
  const handleConfirm = () => {
    const friend = preSelectedFriend || mockFriends.find(f => f.id === selectedFriend);
    ...
    <div className={`min-h-screen p-6 transition-colors duration-300 ${
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className={`rounded-3xl p-6 shadow-sm ${
          ...
          {preSelectedFriend ? (
            <div className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl border-2 border-teal-500/50 ${
              isDark ? 'bg-slate-700/50' : 'bg-teal-50/50'
            }`}>
              <div className="flex items-center justify-center w-12 h-12 rounded-full bg-teal-500 text-white font-bold text-lg shadow-inner">
        ...
        <button
          ...
          disabled={!habitName || (!selectedFriend && !preSelectedFriend)}
          className={`w-full group relative overflow-hidden bg-gradient-to-br from-teal-600 to-cyan-700 rounded-3xl p-6 shadow-lg transition-all duration-300 ${
            !habitName || (!selectedFriend && !preSelectedFriend)
